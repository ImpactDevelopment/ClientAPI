...
public final class ClientAPIUtils {
	...
	public static <T> T[] concat(T[]... arrays) {
		if (arrays.length < 2) throw new ArraySizeException(
		    "At least 2 arrays should be supplied");

		T[] result = arrays[0];
		for (int i = 1; i < arrays.length; i++) {
			T[] newArray =
			    Arrays.copyOf(result, result.length + arrays[i].length);
			System.arraycopy(arrays[i], 0, newArray, result.length,
			    arrays[i].length);
			result = newArray;
		}
	...
	/**
	 * Gets and returns the class that called the method that is calling this
	 * method.
	 *
	 ...
	 * the array. If not, the first object present in the array will be returned
	 * if the size of the array is greater than 1, if not, the specified Object
	 * will be returned.
	 *
	...
	/**
	 * Determines if the specified array of generics contains matching members.
	 *
	...
	public static <T> boolean matchingMembers(T... objects) {
		...
		for (T object : objects) {
			if (!set) baseline = object;
			if (object != baseline) return false;
			set = true;
		}
		return true;
	}
	...
	public static Collection<Class<?>>
	    getCategories(Manager<Module> moduleManager) {
	 ...
	 */
	public static Collection<Class<?>>
	    getCategories(Manager<Module> moduleManager, boolean sort) {
		LinkedHashSet<Class<?>> categories = new LinkedHashSet<>();
		moduleManager.getData().stream().map(Module::getType)
