...
public final class PluginFinder implements Helper {
    ...
    /**
     * Finds the plugin list, the callback is called when the request times out
     * or the list has been found.
     *
    ...
    public final void find(Consumer<PResponse> callback, long timeout) {
        ...
        mc.player.connection
            .sendPacket(new CPacketTabComplete("/", BlockPos.ORIGIN, false));
    }

    @EventHandler
    private final Listener<TickEvent> tickListener = new Listener<>(event -> {
        if (!packetTimer.delay(timeout)) return;

        ClientAPI.EVENT_BUS.unsubscribe(this);
        callback
            .accept(new PResponse("Request timed out after " + timeout + "ms"));
    });

    @EventHandler
    private final Listener<PacketEvent.Receive> packetListener =
        new Listener<>(event -> {
            SPacketTabComplete packet = (SPacketTabComplete) event.getPacket();
            Set<String> plugins = Sets.newLinkedHashSet();

            Arrays.stream(packet.getMatches()).filter(s -> s.contains(":"))
                .forEach(s -> {
    ...
    public static class PResponse {
        ...
        public final Set<String> getPlugins() {
            if (result != SUCCESS) throw new InvalidActionException(
                "Cannot get plugins that were retrieved unless response type is SUCCESS");
        ...
        public final String getError() {
            if (result != FAILURE) throw new InvalidActionException(
                "Cannot get error that occured unless response type is FAILURE");
