...
public final class GlUtils {
    ...
    static {
        ...
        ClientAPI.EVENT_BUS.subscribe(new Object() {
            ...
            @EventHandler
            private final Listener<Render3DEvent> render3DListener =
                new Listener<>(event -> {
                    glGetFloat(GL_MODELVIEW_MATRIX,
                        (FloatBuffer) MODELVIEW.clear());
                    glGetFloat(GL_PROJECTION_MATRIX,
                        (FloatBuffer) PROJECTION.clear());
    ...
    /**
     * Rotates on the X axis at the X, Y, and Z coordinates that are provided.
     *
    ...
    /**
     * Rotates on the Y axis at the X, Y, and Z coordinates that are provided.
     *
     * @param angle The amount being rotated
     * @param x The x position being rotated on
     * @param y The y position being rotated on
    ...
    /**
     * Rotates on the Z axis at the X, Y, and Z coordinates that are provided.
     *
     * @param angle The amount being rotated
     * @param x The x position being rotated on
     * @param y The y position being rotated on
    ...
    /**
     * Converts the specified X, Y, and Z position to the 2D projected position.
     * The returned result is a Vec3 containing the X and Y position, to
     * represent the position on-screen, and a Z position that can be used to
     * indicate whether or not the projected position
     *
     * @return Screen projected coordinates
     */
    public static Vec3 toScreen(double x, double y, double z) {
        ...
        if (result) { return new Vec3(screenCoords.get(0),
            Display.getHeight() - screenCoords.get(1), screenCoords.get(2)); }
        return null;
    }
    ...
    /**
     * Converts the specified X, Y, and Z screen positions to a position in the
     * world, that through some fancy maths (raytracing) can be used get the
     * actual block position of the conversion
     *
     * @return World projected coordinates
     */
