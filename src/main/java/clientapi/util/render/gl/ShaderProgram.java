...
public class ShaderProgram extends GLObject {
    ...
    /**
     * Attaches the shader program. Multiple shader programs may not be attached
     * simultaneously.
     */
    ...
    /**
     * May be overriden by implementations of this class. Used to update uniform
     * variable values when the shader is attached.
     */
    protected void update() {}

    /**
     * Attaches a shader to this shader program. This may only be done before
     * the shader has been compiled via the gen() method and there isn't already
     * a shader with the same type.
     *
     * @param shader The shader
     * @return Whether or not the operation was a success
     */
    public final boolean attachShader(Shader shader) {
        if (isGen() || getShader(shader.getType()) != null) return false;
    ...
    /**
     * Finds and returns a shader attached to this program with the specified
     * type, if there is one. Returns 'null' if a shader isn't found.
     *
     * @param type The type of shader
     * @return The shader with the specified type
     */
    public final Shader getShader(GLShaderType type) {
        return shaders.stream().filter(shader -> shader.getType() == type)
