...
public final class RenderUtils {
	...
	public static final Tessellator tessellator =
	    Tessellator.createExpanding(4, 1, 2);
	...
	/**
	 * Called before rendering. Enables blending, line smoothing, disables 2D
	 * texturing, depth and the depth mask.
	 *
	...
	public static void setupClientState(GLClientState state, boolean enabled) {
		csBuffer.clear();
		if (state.ordinal() > 0) csBuffer.add(state.cap);
	...
	public static void drawLine(Vec3 start, Vec3 end, float width) {
		drawLine((float) start.getX(), (float) start.getY(),
		    (float) start.getZ(), (float) end.getX(), (float) end.getY(),
		    (float) end.getZ(), width);
	}
	 ...
	 * @param width Line width
	 */
	public static void drawLine(float x, float y, float z, float x1, float y1,
	    float z1, float width) {
		glLineWidth(width);

		setupRender(true);
		setupClientState(GLClientState.VERTEX, true);
		tessellator.addVertex(x, y, z).addVertex(x1, y1, z1)
		    .draw(GL_LINE_STRIP);
	 ...
	 */
	public static void drawFlippedTexturedRect(float x1, float y1, float x2,
	    float y2) {
		setupClientState(GLClientState.TEXTURE, true);

		tessellator.addVertex(x1, y2, 0).setTexture(0, 0).addVertex(x2, y2, 0)
		    .setTexture(1, 0).addVertex(x2, y1, 0).setTexture(1, 1)
		    .addVertex(x1, y1, 0).setTexture(0, 1).draw(GL_QUADS);
	 ...
	 * @param y2 Bottom corner Y of the rectangle
	 */
	public static void drawReflectedTexturedRect(float x1, float y1, float x2,
	    float y2) {
		setupClientState(GLClientState.TEXTURE, true);

		tessellator.addVertex(x1, y2, 0).setTexture(1, 0).addVertex(x2, y2, 0)
		    .setTexture(1, 0).addVertex(x2, y1, 0).setTexture(0, 1)
		    .addVertex(x1, y1, 0).setTexture(1, 1).draw(GL_QUADS);
	...
	public static void rectangle(float x1, float y1, float x2, float y2,
		...
		setupRender(true);
		setupClientState(GLClientState.VERTEX, true);
		tessellator.addVertex(x1, y2, 0).addVertex(x2, y2, 0)
		    .addVertex(x2, y1, 0).addVertex(x1, y1, 0).draw(GL_QUADS);
		setupClientState(GLClientState.VERTEX, false);
		setupRender(false);
	}
	...
	public static void rectangleBordered(float x1, float y1, float x2, float y2,
	    int borderColor, int internalColor) {
		rectangleBordered(x1, y1, x2, y2, 0.5F, borderColor, internalColor);
	}

	public static void rectangleBordered(float x1, float y1, float x2, float y2,
	    float width, int borderColor, int internalColor) {
		rectangle(x1 + width, y1 + width, x2 - width, y2 - width,
		    internalColor);
	...
	public static void rectangleGradient(float x1, float y1, float x2, float y2,
	    int c1, int c2) {
		rectangleGradient(x1, y1, x2, y2, new int[] {c1, c2});
	}

	private static void rectangleGradient(float x1, float y1, float x2,
	    float y2, int[] color) {
		if (color.length == 0) throw new RuntimeException(
		    "At least one set of colors should be supplied");
		...
		tessellator.setColor(c1).addVertex(x1, y2, 0).setColor(c2)
		    .addVertex(x2, y2, 0).setColor(c3).addVertex(x2, y1, 0).setColor(c4)
		    .addVertex(x1, y1, 0).draw(GL_QUADS);
	...
	public static void rectangleBorderedGradient(float x1, float y1, float x2,
	    float y2, int border, int c1, int c2) {
		rectangleBorderedGradient(x1, y1, x2, y2, border, c1, c2, 0.5F);
	}

	public static void rectangleBorderedGradient(float x1, float y1, float x2,
	    float y2, int border, int c1, int c2, float width) {
		rectangleBorderedGradient(x1, y1, x2, y2, new int[] {c1, c2},
		    new int[] {border}, width);
	}

	public static void rectangleBorderedGradient(float x1, float y1, float x2,
	    float y2, int[] fill, int[] outline, float width) {
		rectangleOutlinedGradient(x1, y1, x2, y2, outline, width);
		rectangleGradient(x1 + width, y1 + width, x2 - width, y2 - width, fill);
	}

	public static void rectangleOutlinedGradient(float x1, float y1, float x2,
	    float y2, int[] color, float width) {
