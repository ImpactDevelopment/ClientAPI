...
public final class Colors implements Helper {
    ...
    public static float[] getColor(int hex) {
        return new float[] {(hex >> 16 & 0xFF) / 255F, (hex >> 8 & 0xFF) / 255F,
            (hex & 0xFF) / 255F, (hex >> 24 & 0xFF) / 255F};
    }
     ...
     * @param perc Amount of the first color that remains. 1.0 = 100% first
     *            color 0.5 = equally blended 0.0 = 100% second color
     * @return
     */
    public static int blend(int c1, int c2, float perc) {
        ...
        return Colors.getColor(color1[0] * perc + color2[0] * inverse,
    ...
    public static int getColor(float[] color) {
        if (color.length >= 3) return getColor(color[0], color[1], color[2],
    ...
    public static int getColor(float r, float g, float b, float a) {
        return getColor((int) (r * 255F), (int) (g * 255F), (int) (b * 255F),
            (int) (a * 255F));
    }
    ...
    /**
     * Returns the current rainbow hex with custom saturation and brightness.
     *
    ...
    /**
     * Returns the current rainbow hex. An extra parameter, offset is used to
     * offset the System current millisecond time
     *
    ...
    /**
     * Returns the current rainbow hex with custom saturation and brightness. An
     * extra parameter, offset is used to offset the System current millisecond
     * time
     *
     * @param saturation The color saturation applied (0.0 -> 1.0)
     * @param brightness The color brightness applied (0.0 -> 1.0)
    ...
    /**
     * Returns a random color, that has an RGB color range of 0 to 255
     *
    ...
    /**
     * Returns a random color that has the specified RGB color range, as a float
     * (0.0 to 1.0)
     *
    ...
    /**
     * Returns a random color that has the specified RGB color range, as an
     * integer (0 to 255)
     *
     * @param min Minimum R/G/B
     * @param max Maximum R/G/B
    ...
    public static int random(int min, int max) {
        min = clamp(min);
        max = clamp(max);
        return Colors.getColor(MathUtils.random(min, max),
            MathUtils.random(min, max), MathUtils.random(min, max));
    }

    /**
     * Clamps a float color channel value to be within the 0.0 to 1.0 bounds.
     *
    ...
    /**
     * Clamps a integer color channel value to be within the 0 to 255 bounds.
     *
     * @param val Color value
     * @return Clamped value
     */
