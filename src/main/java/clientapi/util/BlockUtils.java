...
public final class BlockUtils implements Helper {
	...
	/**
	 * Gets the first block colliding with the bounding box of the specified
	 * entity.
	 *
	 * @param e The entity
	 * @return The block, returns null if the world isn't initialized
	...
	/**
	 * Gets the first block colliding with the offset bounding box of the
	 * specified entity.
	 *
	 * @param e The entity
	...
	public static Block getBlock(Entity e, Vec3 offset) {
		return getBlock(e.getEntityBoundingBox().offset(offset.getX(),
		    offset.getY(), offset.getZ()));
	}

	/**
	 * Gets the first block that collides with the specified bounding box.
	 *
	...
	public static Block getBlock(AxisAlignedBB bb) {
		int y = (int) bb.minY;
		for (int x = (int) Math.floor(bb.minX); x < (int) Math.floor(bb.maxX)
		    + 1; x++) {
			for (int z =
			    (int) Math.floor(bb.minZ); z < (int) Math.floor(bb.maxZ)
			        + 1; z++) {
