...
/**
 * Manager used to store arrays of information. Just a way to keep things
 * consistent.
 *
 * @see Loadable
 * @see Saveable
    ...
    public final <I extends T> I get(Class<I> clazz) {
        if (!(this instanceof AbstractManager))
            throw new ActionNotSupportedException(
                "Access via class is not supported by this Manager, does not implement AbstractManager.");

        if (clazz == null) return null;

        return (I) classCache.computeIfAbsent((Class<T>) clazz,
            c -> this.data.stream().filter(data -> data.getClass().equals(c))
