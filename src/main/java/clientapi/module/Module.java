...
/**
 * The base for all modules. Contains the data for values, properties, type,
 * keybind, name and description.
 *
 * @see IModule
 * @see Category
 * @author Brady
...
public abstract class Module extends Node implements IModule {
    ...
    public Module() {
        if (!self.isAnnotationPresent(Mod.class)) throw new ModuleInitException(
            "@Mod annotation must be present if required parameters aren't passed through constructor");
    ...
    private void setup(String name, String description, int bind) {
        ...
        if (ClientAPIUtils.containsNull(name, description, type))
            throw new NullPointerException(
                "One or more Mod members were null!");
    }
    ...
    /**
     * Sets the module's mode to the specified mode. Null will be returned if
     * the mode is unable to be set.
     *
    ...
    public final ModuleMode setMode(ModuleMode mode) {
        ...
        if (mode == null || mode.getParent() != this) return null;

        if (this.mode != null) this.mode.setState(false);
    ...
    /**
     * Sets the module's mode to the mode in the specified index. An
     * IndexOutOfBoundsException will be thrown if the index is less than 0 or
    ...
    /**
     * Sets the module's mode from the mode's name. Null will be returned if
     * there isn't a mode with the specified name
     *
    ...
    /**
     * Gets a mode that belongs to this module from the mode's name
     *
    ...
    public final ModuleMode getMode(String name) {
        ...
        return this.modes.stream()
            .filter(mode -> mode.getName().equalsIgnoreCase(name)).findFirst()
            .orElse(null);
    }

    /**
     * Returns the list of modes that this module has, null will be returned if
     * this module doesn't have any modes.
     *
    ...
    /**
     * Switches the mode to the mode following the current mode in the list of
     * modes.
     *
     * @return The new mode
     */
    public final ModuleMode nextMode() {
        ...
        if (++index > this.modes.size() - 1) index = 0;

        return setMode(index);
    }

    /**
     * Switched the mode to the mode preceding the current mode in the list of
     * modes.
     *
     * @return The new mode
     */
    ...
    /**
     * Called when mode related actions are carried out, throws an
     * {@code ActionNotSupportedException} if modes aren't supported by this
     * module.
     */
    private void checkModes() {
        if (!hasModes()) throw new ActionNotSupportedException(
            "Cannot use mode required actions when modes aren't supported");
    }
    ...
    public final void setState(boolean state) {
        ...
        if (hasModes() && mode != null) mode.setState(state);
    }
