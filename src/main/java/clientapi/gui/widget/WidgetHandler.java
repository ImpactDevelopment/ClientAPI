...
public final class WidgetHandler {
	...
	/**
	 * When {@code true}, a white outline will be rendered around all widget
	 * areas,
	 */
	...
	/**
	 * Renders all of the widgets on a screen with the specified scaled
	 * resolution using the specified font renderer.
	 *
	...
	public final void draw(FontRenderer font, ScaledResolution sr) {
		...
		widgetMap.forEach((pos, widgets) -> {
			...
			widgets.forEach(widget -> {
				float mP = (widget.getAlignment().getValue() + 0.5F != 0.0F)
				    ? 1.0F : 0.0F;
				...
				glPushMatrix();
				glTranslatef(
				    widget.getAlignment().getValue() * widget.getWidth() + xP,
				    yP, 0.0F);
				if (outlines)
				    RenderUtils.rectangleBordered(0, 0, widget.getWidth(),
				        widget.getHeight(), 0xFFFFFFFF, 0x00000000);
	...
	/**
	 * Adds a widget, assuming that the specified widget isn't already
	...
	/**
	 * Removes a widget, assuming that the specified widget is already
	 * registered with this handler.
	 *
	 * @param widget The widget to remove
	 */
	public final void remove(Widget widget) {
		if (widgets.contains(widget)) widgets.remove(widget);
	}
